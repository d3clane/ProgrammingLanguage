...
enum class TokensArrErrors
{
    ...
    TOKENS_ARR_MEMORY_ALLOCATION_ERROR,
    TOKENS_ARR_EMPTY_ERR, 
    TOKENS_ARR_IS_NULLPTR,
    TOKENS_ARR_CAPACITY_OUT_OF_RANGE,
    TOKENS_ARR_SIZE_OUT_OF_RANGE,
    TOKENS_ARR_INVALID_CANARY, 
    TOKENS_ARR_INVALID_DATA_HASH,
    TOKENS_ARR_INVALID_STRUCT_HASH,
};
    ...
    TokensArrErrors TokensArrCtor(TokensArrType* const stk, const size_t capacity = 0, 
...
void TokensArrDump(const TokensArrType* stk, const char* const fileName, 
...
static inline bool TokensArrIsEmpty(const TokensArrType* stk)
{
    assert(stk);
    assert(stk->data);
