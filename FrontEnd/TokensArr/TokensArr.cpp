...
static TokensArrErrors TokensArrRealloc(TokensArrType* tokensArr, bool increase);

static inline ElemType* MovePtr(ElemType* const data, const size_t moveSz, const int times);

static inline size_t TokensArrGetSizeForCalloc(TokensArrType* const tokensArr);

static void TokensArrDataFill(TokensArrType* const tokensArr);

static inline bool TokensArrIsFull(TokensArrType* tokensArr);

static inline bool TokensArrIsTooBig(TokensArrType* tokensArr);
...
//--------CANARY PROTECTION----------
    ...
    static inline ElemType* GetAfterFirstCanaryAdr(const TokensArrType* const tokensArr)
    {
        return MovePtr(tokensArr->data, sizeof(CanaryType), 1);
    }

    static inline ElemType* GetFirstCanaryAdr(const TokensArrType* const tokensArr)
    {
        return MovePtr(tokensArr->data, sizeof(CanaryType), -1);
    }

    static inline ElemType* GetSecondCanaryAdr(const TokensArrType* const tokensArr)
    {
...
//-----------HASH PROTECTION---------------
    ...
    static inline HashType CalcDataHash(const TokensArrType* tokensArr)
    {
        return tokensArr->HashFunc(tokensArr->data, tokensArr->capacity * sizeof(ElemType), 0);        
    }

    static inline void UpdateDataHash(TokensArrType* tokensArr)
    {
        tokensArr->dataHash = CalcDataHash(tokensArr);      
    }

    static inline void UpdateStructHash(TokensArrType* tokensArr)
    {
        tokensArr->structHash = 0;                                  
        tokensArr->structHash = MurmurHash(tokensArr, sizeof(*tokensArr));        
    }
...
//---------------
    ...
    {                                           \
        TokensArrErrors tokensArrErr = TokensArrVerify(tokensArr); \
                                                \
        if (tokensArrErr != TokensArrErrors::TOKENS_ARR_NO_ERR)                      \
        {                                       \
...
do                                      \
{                                       \
    if (ERR != TokensArrErrors::TOKENS_ARR_NO_ERR)                            \
        return ERR;                     \
} while (0)
...
#ifdef TOKENS_ARR_HASH_PROTECTION
TokensArrErrors TokensArrCtor(TokensArrType* const tokensArr, const size_t capacity, 
                          const HashFuncType HashFunc)
{
    ...
    TokensArrErrors errors = TokensArrErrors::TOKENS_ARR_NO_ERR;
    tokensArr->size = 0;
    ...
    {
        TokensArrPrintError(TokensArrErrors::TOKENS_ARR_MEMORY_ALLOCATION_ERROR);  
        return TokensArrErrors::TOKENS_ARR_MEMORY_ALLOCATION_ERROR;   
    }
...
TokensArrErrors TokensArrDtor(TokensArrType* const tokensArr)
{
    assert(tokensArr);
    ...
    return TokensArrErrors::TOKENS_ARR_NO_ERR;
}

TokensArrErrors TokensArrPush(TokensArrType* tokensArr, const ElemType val)
{
    assert(tokensArr);
    
    TOKENS_ARR_CHECK(tokensArr);

    TokensArrErrors tokensArrReallocErr = TokensArrErrors::TOKENS_ARR_NO_ERR;
...
TokensArrErrors TokensArrVerify(TokensArrType* tokensArr)
{
    assert(tokensArr);
    ...
    {  
        TokensArrPrintError(TokensArrErrors::TOKENS_ARR_CAPACITY_OUT_OF_RANGE);
        return TokensArrErrors::TOKENS_ARR_CAPACITY_OUT_OF_RANGE;
    }

    if (tokensArr->size > tokensArr->capacity)
    {
        TokensArrPrintError(TokensArrErrors::TOKENS_ARR_SIZE_OUT_OF_RANGE);
        return TokensArrErrors::TOKENS_ARR_SIZE_OUT_OF_RANGE;
    }
    ...
    (
        ...
        {
            TokensArrPrintError(TokensArrErrors::TOKENS_ARR_INVALID_CANARY);
            return TokensArrErrors::TOKENS_ARR_INVALID_CANARY;
        }
    ...
    (
        ...
        {
            TokensArrPrintError(TokensArrErrors::TOKENS_ARR_INVALID_DATA_HASH);
            return TokensArrErrors::TOKENS_ARR_INVALID_DATA_HASH;
        }
    )
    ...
    (
        ...
        {
            TokensArrPrintError(TokensArrErrors::TOKENS_ARR_INVALID_STRUCT_HASH);
            tokensArr->structHash = prevStructHash;
            return TokensArrErrors::TOKENS_ARR_INVALID_STRUCT_HASH;
        }
    )
...
void TokensArrDump(const TokensArrType* tokensArr, const char* const fileName, 
...
TokensArrErrors TokensArrRealloc(TokensArrType* tokensArr, bool increase)
{
    assert(tokensArr);
    ...
    if (!increase) 
        FillArray(tokensArr->data + tokensArr->capacity, tokensArr->data + tokensArr->size, TOKENS_ARR_POISON);
    ...
    if (increase)
        FillArray(tokensArr->data + tokensArr->size, tokensArr->data + tokensArr->capacity, TOKENS_ARR_POISON);
...
static void TokensArrDataFill(TokensArrType* const tokensArr)
{
    ...
    FillArray(tokensArr->data, tokensArr->data + tokensArr->capacity, TOKENS_ARR_POISON);
...
void TokensArrPrintError(TokensArrErrors error)
{
    ...
    switch(error)
    {
        case TokensArrErrors::TOKENS_ARR_CAPACITY_OUT_OF_RANGE:
        ...
        case TokensArrErrors::TOKENS_ARR_EMPTY_ERR:
            ...
            break;
        case TokensArrErrors::TOKENS_ARR_SIZE_OUT_OF_RANGE:
            LOG_ERR("TokensArr size is out of range.\n");
            break;
        case TokensArrErrors::TOKENS_ARR_MEMORY_ALLOCATION_ERROR:
            LOG_ERR("Couldn't allocate more memory for tokensArr.\n");
            break;
        case TokensArrErrors::TOKENS_ARR_INVALID_CANARY:
            LOG_ERR("TokensArr canary is invalid.\n");
            break;
        case TokensArrErrors::TOKENS_ARR_INVALID_DATA_HASH:
            LOG_ERR("TokensArr data hash is invalid.\n");
            break;
        case TokensArrErrors::TOKENS_ARR_INVALID_STRUCT_HASH:
            LOG_ERR("TokensArr struct hash is invalid.\n");

        case TokensArrErrors::TOKENS_ARR_NO_ERR:
        default:
            break;
    }
